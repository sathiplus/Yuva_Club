...
$quickChallengePolicies=[];$quickChallengeEvaluations=[];try{$quickChallengePolicies=quick_challenge_evaluation_service()->policies();$quickChallengeEvaluations=quick_challenge_evaluation_service()->adminEvaluations($competitionActor);}catch(Throwable $error){error_log('YUVA Quick Challenge scoring admin view unavailable correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}
...
$growthDefinitions=[];$growthMetrics=[];$growthStudent=normalize_yuva_id((string)($_GET['growth_student']??''));$growthProfile=null;try{$growthDefinitions=growth_profile_service()->definitions($admin);$growthMetrics=growth_profile_service()->betaMetrics($admin);if($growthStudent!=='')$growthProfile=growth_profile_service()->forAdmin($admin,$growthStudent);}catch(Throwable $error){error_log('YUVA master growth oversight unavailable correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}
...
$subscriptionNotice=(string)($_SESSION['subscription_notice']??'');$subscriptionError=(string)($_SESSION['subscription_error']??'');unset($_SESSION['subscription_notice'],$_SESSION['subscription_error'],$_SESSION['subscription_invitation_code']);
...
<div class="master-shell">
  ...
  <main class="master-main" id="master-main">
    ...
    <section class="master-panel" id="leadership-reviews"><div class="master-panel-heading"><div><p class="master-kicker">Human approval</p><h2>Global Leadership Review Queue</h2><p>AI and system evidence may establish eligibility, but only a human decision can change an approved level.</p></div></div><?php if($leadershipAdminNotice!==''): ?><div class="form-status success"><?php echo e($leadershipAdminNotice); ?></div><?php endif; ?><?php if($leadershipAdminError!==''): ?><div class="form-status error"><?php echo e($leadershipAdminError); ?></div><?php endif; ?>
      <?php foreach($leadershipRows as $leadershipStudentId=>$leadershipRow): $progress=$leadershipRow['progress'];$identity=public_student_identity((string)$leadershipStudentId);$avatar=\YuvaClub\Identity\PublicStudentIdentity::avatar($identity['avatar_code']); ?><article class="form-card"><h3><?php echo e($avatar['icon'].' '.($identity['handle']?:$leadershipStudentId)); ?> <small><?php echo e((string)$leadershipStudentId); ?></small></h3><p><?php echo e((string)$progress['current_level']); ?> → <?php echo e((string)($progress['target_level']??'Continued mentorship')); ?> · <strong><?php echo e((string)$progress['status']); ?></strong> · Rule <?php echo e((string)$progress['rule_version']); ?></p><ul><?php foreach($progress['requirements'] as $requirement): ?><li><?php echo !empty($requirement['complete'])?'✓':'○'; ?> <?php echo e((string)$requirement['label']); ?> (<?php echo e((string)$requirement['actual']); ?>/<?php echo e((string)$requirement['required']); ?>)</li><?php endforeach; ?></ul>
      <h4>Completed presentations</h4><?php if(($leadershipRow['submissions']??[])===[]): ?><p>No completed presentation submissions.</p><?php else: ?><?php foreach($leadershipRow['submissions'] as $submission): ?><article><p>Presentation <?php echo e((string)$submission['id']); ?> · <?php echo e((string)($submission['verification_status']??'Awaiting Human Verification')); ?></p><?php if(empty($submission['verification_guid'])): ?><form action="admin-presentation-verification.php" method="post"><?php echo csrf_field(); ?><input type="hidden" name="yuva_id" value="<?php echo e((string)$leadershipStudentId); ?>"><input type="hidden" name="submission_id" value="<?php echo e((string)$submission['id']); ?>"><label>Optional verification note<textarea name="note" maxlength="2000"></textarea></label><button class="button primary" type="submit">Verify Presentation</button></form><?php else: ?><form action="admin-presentation-verification.php" method="post"><?php echo csrf_field(); ?><input type="hidden" name="action" value="revoke"><input type="hidden" name="verification_guid" value="<?php echo e((string)$submission['verification_guid']); ?>"><input type="hidden" name="row_version" value="<?php echo e((string)$submission['row_version']); ?>"><label>Revocation reason<textarea name="reason" required maxlength="2000"></textarea></label><button class="button ghost" type="submit">Revoke Verification</button></form><?php endif; ?></article><?php endforeach; ?><?php endif; ?>
    ...
    <section class="form-card" id="growth-oversight">
      <p class="eyebrow">My Growth oversight</p><h2>Meaningful achievement definitions</h2>
      <p>Definitions are system-determined. Administrative corrections are audited; practice scores never change Leadership or official Competition outcomes.</p>
      <form method="get"><label>Inspect student YUVA ID <input name="growth_student" value="<?php echo e($growthStudent); ?>"></label><button class="button ghost">Inspect growth evidence</button></form>
      <?php if($growthMetrics!==[]): ?><p>Beta: <?php echo e((string)$growthMetrics['students_attempted']); ?> attempted · <?php echo e((string)$growthMetrics['students_repeated']); ?> repeated · <?php echo e((string)$growthMetrics['students_improved']); ?> improved · <?php echo e((string)$growthMetrics['students_returned_another_week']); ?> returned another week</p><?php endif; ?>
      <?php foreach($growthDefinitions as$definition): ?><form action="admin-growth-action.php" method="post" class="form-card"><?php echo csrf_field(); ?><input type="hidden" name="action" value="definition_status"><input type="hidden" name="achievement_code" value="<?php echo e((string)$definition['achievement_code']); ?>"><strong><?php echo e((string)$definition['display_name']); ?></strong> · <?php echo e((string)$definition['status']); ?><button class="button ghost" name="status" value="<?php echo $definition['status']==='Active'?'Disabled':'Active'; ?>"><?php echo $definition['status']==='Active'?'Disable':'Enable'; ?></button></form><?php endforeach; ?>
      <?php if(is_array($growthProfile)): ?><h3><?php echo e($growthStudent); ?> growth summary</h3><p><?php echo e((string)$growthProfile['summary']['challenges_completed']); ?> challenges · <?php echo e((string)$growthProfile['summary']['personal_bests']); ?> Personal Bests · <?php echo e((string)$growthProfile['summary']['benchmarks_beaten']); ?> benchmarks</p><?php foreach($growthProfile['achievements'] as$achievement): ?><form action="admin-growth-action.php" method="post" class="form-card"><?php echo csrf_field(); ?><input type="hidden" name="action" value="revoke"><input type="hidden" name="achievement_guid" value="<?php echo e((string)$achievement['achievement_guid']); ?>"><strong><?php echo e((string)$achievement['display_name']); ?></strong><p>Evidence: <?php echo e((string)$achievement['source_type']); ?> / <?php echo e((string)$achievement['source_reference']); ?></p><label>Correction reason <input name="reason" required maxlength="500"></label><button class="button ghost">Revoke with audit</button></form><?php endforeach; ?><?php endif; ?>
    </section>
    </section>
  </main>
