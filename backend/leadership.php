...
final class LeadershipEligibilityService
{
    ...
    {
        $evaluation=$this->pdo->prepare("INSERT dbo.leadership_evidence(student_id,evidence_type,source_type,source_id,[status],approved_by_email,approved_by_role,evidence_date,notes) SELECT evaluation.student_id,N'presentation',N'evaluation',CONVERT(NVARCHAR(120),evaluation.id),N'Approved',reviewer.email,N'MasterAdmin',CONVERT(date,evaluation.created_at),N'Verified presentation evaluation' FROM dbo.evaluations evaluation LEFT JOIN dbo.users reviewer ON reviewer.id=evaluation.evaluator_user_id WHERE evaluation.student_id=:student AND NOT EXISTS(SELECT 1 FROM dbo.leadership_evidence evidence WITH(UPDLOCK,HOLDLOCK) WHERE evidence.student_id=evaluation.student_id AND evidence.evidence_type=N'presentation' AND evidence.source_type=N'evaluation' AND evidence.source_id=CONVERT(NVARCHAR(120),evaluation.id))");$evaluation->execute(['student'=>$studentId]);
        $human=$this->pdo->prepare("INSERT dbo.leadership_evidence(student_id,evidence_type,source_type,source_id,[status],approved_by_email,approved_by_role,evidence_date,notes) SELECT evaluation.student_id,N'human_review',N'evaluation',CONVERT(NVARCHAR(120),evaluation.id),N'Approved',reviewer.email,N'MasterAdmin',CONVERT(date,evaluation.created_at),evaluation.feedback FROM dbo.evaluations evaluation LEFT JOIN dbo.users reviewer ON reviewer.id=evaluation.evaluator_user_id WHERE evaluation.student_id=:student AND NOT EXISTS(SELECT 1 FROM dbo.leadership_evidence evidence WITH(UPDLOCK,HOLDLOCK) WHERE evidence.student_id=evaluation.student_id AND evidence.evidence_type=N'human_review' AND evidence.source_type=N'evaluation' AND evidence.source_id=CONVERT(NVARCHAR(120),evaluation.id))");$human->execute(['student'=>$studentId]);
    ...
    private function evidenceCounts(int $studentId,array $requirements): array
    {
        ...
        $presentations=$this->pdo->prepare("SELECT COUNT_BIG(DISTINCT id) FROM dbo.evaluations WHERE student_id=:student");
        $presentations->execute(['student'=>$studentId]);
        $counts['presentations']=max($counts['presentation']??0,(int)$presentations->fetchColumn());
        $reviews=$this->pdo->prepare("SELECT (SELECT COUNT_BIG(*) FROM dbo.ai_mentor_reviews WHERE student_id=:student AND [status]=N'Applied') + (SELECT COUNT_BIG(*) FROM dbo.evaluations WHERE student_id=:student2)");
    ...
    private function rule(int $levelId): ?array { $stmt=$this->pdo->prepare("SELECT TOP(1) rule.*, target.name target_level FROM dbo.leadership_rule_versions rule JOIN dbo.levels target ON target.id=rule.level_to_id WHERE rule.level_from_id=:level AND rule.rule_version=:version AND rule.active=1 ORDER BY rule.effective_from DESC,rule.id DESC"); $stmt->execute(['level'=>$levelId,'version'=>self::RULE_VERSION]); $row=$stmt->fetch(); return is_array($row)?$row:null; }
