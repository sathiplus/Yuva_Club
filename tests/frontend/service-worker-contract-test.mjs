...
assert.ok(sw.includes("const CACHE_NAME = 'yuva-club-release-1-0-1-v1'"), 'Cache name must be release-1.0.1 value');
assert.ok(sw.includes('/assets/site.css?v=release-1.0.1-20260731'), 'service worker must precache versioned site.css');
assert.ok(sw.includes('/assets/public-site.css?v=release-1.0.1-20260731'), 'service worker must precache versioned public-site.css');
assert.ok(sw.includes('/assets/app.js?v=release-1.0.1-20260731'), 'service worker must precache versioned app.js');
...
assert.ok(sw.includes("if (request.mode === 'navigate')"), 'navigation must keep network-first behavior');
assert.ok(sw.includes('url.pathname.endsWith(\'.php\')'), 'php requests must stay network-only');
