...
$migration=$read('database/17-parent-password-authentication.azure-sql.sql');$rollback=$read('database/17-parent-password-authentication-rollback.sql');$service=$read('backend/authentication/ParentCredentialService.php');$auth=$read('backend/authentication/ParentAuthentication.php');$authenticationService=$read('backend/authentication/AuthenticationService.php');$workflow=$read('backend/authentication/ParentLoginWorkflow.php');$login=$read('parent-login.php');$certificate=$read('certificate.php');
...
parent_security_assert(
    str_contains($authenticationService, 'parents->authorizedChildRecord'),
    'Parent-linked child authorization must not reuse Student password-session validation.'
);
