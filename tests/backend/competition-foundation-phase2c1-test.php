...
competition_check(str_contains($service,'competition.row_version AS row_version_binary'),'Managed challenge rowversion must use one unambiguous internal SQLSRV result column.');
competition_check(str_contains($service,"\$row['row_version']=normalize_sqlsrv_rowversion_token(\$row['row_version_binary']??null)")&&str_contains($service,"unset(\$row['row_version_binary'])"),'Managed challenge rowversion must be normalized and raw bytes removed at the PHP boundary.');
