...
try{
 ...
 $expired=$service->issueToken($email,'password_reset');$expiredId=(int)explode('.',(string)$expired,2)[0];$pdo->prepare("UPDATE dbo.parent_authentication_tokens SET created_at=DATEADD(MINUTE,-2,SYSUTCDATETIME()),expires_at=DATEADD(MINUTE,-1,SYSUTCDATETIME()) WHERE id=:id")->execute(['id'=>$expiredId]);if($service->consume((string)$expired,'password_reset','ExpiredParent12!'))throw new RuntimeException('Expired reset accepted.');
