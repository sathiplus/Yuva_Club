...
Assert-FileContains 'portal-lib.php' 'delete_organization_admin_account' 'MasterAdmin must have an audited way to remove test organization admin accounts.'
Assert-FileContains 'portal-lib.php' 'master_archive_organization_student_membership' 'MasterAdmin cleanup must archive organization memberships without deleting global student records.'
Assert-FileContains 'admin-organization-student-actions.php' 'master_archive_organization_student_membership' 'MasterAdmin membership cleanup must use the archive helper.'
Assert-FileNotContains 'admin-organization-student-actions.php' 'unset\(\$students|unlink\(|DELETE FROM students' 'MasterAdmin membership cleanup must not delete global student records.'
