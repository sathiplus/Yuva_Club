...
$quickChallengeResults=[];try{$quickChallengeResults=quick_challenge_evaluation_service()->resultsForStudent($studentId);}catch(Throwable $error){error_log('YUVA parent Quick Challenge results unavailable correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}
$growthProfile=null;try{$growthProfile=growth_profile_service()->forParent($_SESSION,$studentId);}catch(Throwable $error){error_log('YUVA parent growth view unavailable correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}
try {
...
<main class="parent-experience" id="parent-main" tabindex="-1">
  ...
  <section class="parent-section" id="parent-growth" aria-labelledby="parent-growth-title">
    ...
    <div class="parent-growth-grid">
      ...
      <article class="parent-metric parent-metric-rubric official-result"><span class="parent-card-icon" aria-hidden="true"></span><p>Official Rubric Score</p><strong><?php echo e((string) $rubricScore); ?><small>/100</small></strong><small>Human-reviewed presentation evaluation—not an AI Practice Score</small></article>
    </div>
    <?php if(is_array($growthProfile)): $growthSummary=$growthProfile['summary']; ?><article class="parent-progress-card"><div class="parent-card-heading"><div><p class="eyebrow">Child Growth</p><h2>Practice progress</h2><p>Private, linked-child-only growth from meaningful activity.</p></div></div><p><strong><?php echo e((string)$growthSummary['challenges_completed']); ?></strong> challenges · <strong><?php echo e((string)$growthSummary['personal_bests']); ?></strong> Personal Bests · <strong><?php echo e((string)$growthSummary['benchmarks_beaten']); ?></strong> benchmarks beaten · <strong><?php echo e((string)$growthSummary['consistency_weeks']); ?></strong> consistent weeks</p><p><?php echo e((string)$growthProfile['next_action']['text']); ?></p><h3>Achievements</h3><ul><?php foreach($growthProfile['achievements'] as$achievement): ?><li><?php echo e((string)$achievement['display_name']); ?></li><?php endforeach; ?></ul></article><?php endif; ?>
    ...
    <?php foreach($quickChallengeResults as$result): if(($result['status']??'')!=='Completed')continue; ?><article class="parent-progress-card practice-result"><div class="parent-card-heading"><div><p class="eyebrow">AI Practice Score · coaching only</p><h2><?php echo e((string)$result['display_name']); ?></h2><p>AI Practice Score <?php echo e((string)$result['total_score']); ?> · Personal Best <?php echo e((string)($result['best_score']??$result['total_score'])); ?></p><p class="form-note">This is practice coaching, not an official competition result. Personal Best is the highest comparable practice score; a benchmark is a private YUVA target, not another student.</p></div></div><p><?php echo (int)$result['total_score']>=(int)$result['benchmark_score']?'Benchmark beaten':'Keep practicing — '.e((string)((int)$result['benchmark_score']-(int)$result['total_score'])).' points to the benchmark'; ?></p><p><strong>Next practice mission:</strong> <?php echo e((string)($result['coaching']['practice_mission']??'')); ?></p></article><?php endforeach; ?>
