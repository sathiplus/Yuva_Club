...
<main class="parent-experience" id="parent-main" tabindex="-1">
  ...
  <section class="parent-section" id="parent-growth" aria-labelledby="parent-growth-title">
    ...
    <?php foreach($quickChallengeResults as$result): if(($result['status']??'')!=='Completed')continue; ?><article class="parent-progress-card"><div class="parent-card-heading"><div><p class="eyebrow">Quick Challenge completed</p><h2><?php echo e((string)$result['display_name']); ?></h2><p>Practice Score <?php echo e((string)$result['total_score']); ?> · Personal Best <?php echo e((string)($result['best_score']??$result['total_score'])); ?></p></div></div><p><?php echo (int)$result['total_score']>=(int)$result['benchmark_score']?'Benchmark beaten':'Keep practicing — '.e((string)((int)$result['benchmark_score']-(int)$result['total_score'])).' points to the benchmark'; ?></p><p><strong>Next practice mission:</strong> <?php echo e((string)($result['coaching']['practice_mission']??'')); ?></p></article><?php endforeach; ?>
