...
$quickChallengePolicies=[];$quickChallengeEvaluations=[];try{$quickChallengeEvaluations=quick_challenge_evaluation_service()->adminEvaluations($admin);}catch(Throwable $error){error_log('YUVA organization Quick Challenge scoring view unavailable correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}
...
$growthStudent=normalize_yuva_id((string)($_GET['growth_student']??''));$growthProfile=null;if($growthStudent!==''){try{$growthProfile=growth_profile_service()->forAdmin($admin,$growthStudent);}catch(Throwable $error){error_log('YUVA organization growth view rejected correlation='.bin2hex(random_bytes(12)).' exception_type='.get_class($error));}}
  ...
  <section class="band">
    ...
    <section class="form-card"><h2>Organization Students</h2>
      ...
      <?php else: ?><div class="table-wrap"><table><thead><tr><th>YUVA Identity</th><th>Name</th><th>Program</th><th>School</th><th>Status</th></tr></thead><tbody>
        <?php foreach ($students as $studentId => $student): $studentRecord = $records[$studentId] ?? []; $identity=public_student_identity((string)$studentId);$avatar=\YuvaClub\Identity\PublicStudentIdentity::avatar($identity['avatar_code']); ?><tr><td><strong><?php echo e($avatar['icon'].' '.($identity['handle']?:$studentId)); ?></strong><br><small><?php echo e((string)$studentId); ?></small></td><td><?php echo e(student_display_name($student)); ?></td><td><?php echo e((string) ($student['Program Group'] ?? '')); ?></td><td><?php echo e((string) ($student['School'] ?? '')); ?></td><td><?php echo e((string) ($studentRecord['approved'] ?? 'Pending')); ?> · <a href="organization-admin.php?growth_student=<?php echo urlencode((string)$studentId); ?>#organization-growth">Growth</a></td></tr><?php endforeach; ?>
      </tbody></table></div><?php endif; ?>
    </section>
    <?php if(is_array($growthProfile)): ?><section class="form-card" id="organization-growth"><p class="eyebrow">Authorized same-organization summary</p><h2>Student Growth · <?php echo e($growthStudent); ?></h2><p><?php echo e((string)$growthProfile['summary']['challenges_completed']); ?> challenges · <?php echo e((string)$growthProfile['summary']['personal_bests']); ?> Personal Bests · <?php echo e((string)$growthProfile['summary']['benchmarks_beaten']); ?> benchmarks · <?php echo e((string)$growthProfile['summary']['leadership_level']); ?> level</p><p><?php echo e((string)$growthProfile['next_action']['text']); ?></p></section><?php endif; ?>
  </section>
</main>
<?php portal_footer(); ?>
